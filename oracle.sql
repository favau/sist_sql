...
SELECT ename,job FROM emp WHERE empno IN(SELECT empno FROM emp WHERE comm>=500 AND empno IN '30');
6)'MANAGER'직업인 사원의 평균 급여보다 높은 사원의 이름과 근무시작일을 오래된 순서부터 출력하시오.
SELECT ename,hiredate FROM emp WHERE hiredate IN(SELECT AVG(sal) FROM emp WHERE job IN 'MANAGER') ASC;
7)직업별 사원수와 평균 급여를 출력하시오.
SELECT COUNT(job),AVG(sal) FROM emp;
8)부서번호가 '30'인 사원의 커미션 유무(O,X로 출력)와 커미션,근무지,직업을 출력하시오.
SELECT e.comm,d.loc,e.job,커미션 FROM emp e INNER JOIN dept d ON e.deptno = d.deptno WHERE e.comm IN (SELECT e.comm FROM emp e, dept d WHERE d.deptno IN '30');
